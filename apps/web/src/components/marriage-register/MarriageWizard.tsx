...
type MarriageRecord = {
  ...
  bridegroomDob?: string | null;
  bridegroomNationality?: string | null;
  bridegroomDomicile?: string | null;
  bridegroomOccupation?: string | null;
  bridegroomMaritalStatus?: string | null;
  bridegroomPreviousSpouse?: string | null;
  brideName?: string | null;
  brideSurname?: string | null;
  brideFatherName?: string | null;
  brideMotherName?: string | null;
  brideDob?: string | null;
  brideNationality?: string | null;
  brideDomicile?: string | null;
  brideOccupation?: string | null;
  brideMaritalStatus?: string | null;
  bridePreviousSpouse?: string | null;
  bann1At?: string | null;
  bann2At?: string | null;
  bann3At?: string | null;
  dispensationNotes?: string | null;
...
function recordToForm(record: MarriageRecord) {
  return {
    ...initial,
    parishId: record.parishId,
    registerNumber: record.registerNumber || '',
    registerYear: String(record.registerYear || new Date().getFullYear()),
    celebratedAt: isoDate(record.celebratedAt),
    placeOfMarriage: record.placeOfMarriage || record.churchName || '',
    churchName: record.churchName || '',
    ministerName: record.ministerName || '',
    parishPriestName: record.parishPriestName || '',
    memberId: record.memberId || '',
    spouseMemberId: record.spouseMemberId || '',
    ...
    bridegroomDob: isoDate(record.bridegroomDob),
    bridegroomNationality: record.bridegroomNationality || initial.bridegroomNationality,
    bridegroomDomicile: record.bridegroomDomicile || '',
    bridegroomOccupation: record.bridegroomOccupation || '',
    bridegroomMaritalStatus: record.bridegroomMaritalStatus || initial.bridegroomMaritalStatus,
    bridegroomPreviousSpouse: record.bridegroomPreviousSpouse || '',
    ...
    brideDob: isoDate(record.brideDob),
    brideNationality: record.brideNationality || initial.brideNationality,
    brideDomicile: record.brideDomicile || '',
    brideOccupation: record.brideOccupation || '',
    brideMaritalStatus: record.brideMaritalStatus || initial.brideMaritalStatus,
    bridePreviousSpouse: record.bridePreviousSpouse || '',
    bann1At: isoDate(record.bann1At),
    bann2At: isoDate(record.bann2At),
    bann3At: isoDate(record.bann3At),
    dispensationNotes: record.dispensationNotes || '',
    ...
    issueCertificate: Boolean(record.certificateId),
  };
}

function buildMarriagePayload(form: typeof initial) {
  return {
    ...
    remarks:
      [form.remarks, form.marriageTime ? `Time of marriage: ${formatTimeLabel(form.marriageTime)}` : '']
        .filter(Boolean)
        .join('\n') || undefined,
...
export function MarriageWizard({ editId }: { editId?: string }) {
  ...
  const existing = useQuery({
    queryKey: ['sacrament', editId],
    queryFn: () => api.get<MarriageRecord>(`/sacraments/${editId}`),
    enabled: Boolean(editId),
  });

  useEffect(() => {
    if (!editId || !existing.data) return;
    setForm(recordToForm(existing.data));
  ...
  const years = useMemo(() => {
    ...
    const selected = Number(form.registerYear);
    if (Number.isFinite(selected) && !base.includes(selected)) base.push(selected);
    return base.sort((a, b) => b - a).map(String);
  }, [form.registerYear]);
  ...
  return (
    <div>
      <PageHeader
        title={isEditing ? 'Edit Marriage Record' : 'Marriage Register'}
        description={
          isEditing
            ? 'Update an existing marriage register entry.'
            : 'Create a new marriage record following parish register format.'
        }
        actions={
          ...
          <CardContent className="space-y-6">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 border-t border-[var(--bcl-border)] pt-4">
              ...
              {step < MARRIAGE_STEPS.length - 1 ? (
                ...
                >
                  {save.isPending ? 'Saving…' : isEditing ? 'Update Marriage' : 'Save Marriage'}
                </Button>
              )}
            </div>
            {save.isError ? (
              <p className="text-sm text-red-600">
                {(save.error as Error)?.message || 'Could not save marriage.'}
              </p>
            ) : null}
            {isEditing && existing.isLoading ? (
              <p className="text-sm text-[var(--bcl-muted)]">Loading marriage record…</p>
            ) : null}
          </CardContent>
        </Card>
